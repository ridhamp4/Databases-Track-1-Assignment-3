...
def create_group():
    ...
    # Atomic: create group + add creator as admin member in single transaction
    group_id = execute_transaction([
...
def join_group(group_id):
    ...
    try:
        ...
        # Duplicate key from concurrent join — already a member
        return jsonify(error='Already a member'), 409
...
def make_admin(group_id, member_id):
    ...
    # Atomic: transfer admin role in single transaction
    execute_transaction([
        ("UPDATE CampusGroup SET AdminID = %s WHERE GroupID = %s", (member_id, group_id)),
