...
def toggle_like(post_id):
    """Toggle like using INSERT/DELETE with duplicate-key handling to prevent race conditions."""
    user_id = int(get_jwt_identity())
    # Try to insert first; if duplicate key, delete instead (atomic toggle)
    conn = get_db(autocommit=False)
    try:
        cursor = conn.cursor()
        try:
            cursor.execute("INSERT INTO PostLike (PostID, MemberID) VALUES (%s,%s)", (post_id, user_id))
            conn.commit()
            cursor.close()
            log_action('LIKE_POST', f"Liked post {post_id}", user=get_current_username())
            return jsonify(liked=True)
        except Exception:
            # Duplicate key — row already exists, so unlike
            conn.rollback()
            cursor.execute("DELETE FROM PostLike WHERE PostID = %s AND MemberID = %s", (post_id, user_id))
            conn.commit()
            cursor.close()
