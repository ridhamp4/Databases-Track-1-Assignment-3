...
def transaction_test():
    """Test endpoint to demonstrate ACID transactions.
    Accepts a list of SQL statements and executes them atomically."""
    ...
    if not statements:
        return {'error': 'No statements provided'}, 400
    ...
    for s in statements:
        if isinstance(s, str):
            ...
            parsed.append((s['sql'], s.get('args', ())))
        elif isinstance(s, list) and len(s) >= 1:
            parsed.append((s[0], s[1] if len(s) > 1 else ()))

    if simulate_failure:
        # Add an intentionally failing statement to test rollback
        parsed.append(("INSERT INTO nonexistent_table VALUES (1)", ()))

    try:
        result = execute_transaction(parsed)
        return {'status': 'committed', 'last_id': result}
    except Exception as e:
